...
// importKeywords 接收 CSV 文本（word,category,match_mode,action[,enabled]）。
func (s *Server) importKeywords(c *gin.Context) {
	...
	for _, rec := range records {
		...
		if len(rec) > 4 && (strings.TrimSpace(rec[4]) == "false" || strings.TrimSpace(rec[4]) == "0") {
			en = false
		}
...
func (s *Server) testPii(c *gin.Context) {
	...
	for _, f := range vd.Findings {
		if f.Type == "pii" {
			findings = append(findings, gin.H{"rule": f.Value,
				"category": strings.TrimPrefix(f.Category, "pii."), "sample": maskSample(f.Value)})
		}
	}
