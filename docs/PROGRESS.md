...
最后更新：2026-09-10（第二十四轮：配额/限流全链路实测——四机制全部生效）

## 本轮迭代变更（第二十四轮）

### 配额与限流实测（无代码变更，验证轮）
本地 8081 实例 + 真实库 + 真实上游（sensenova glm5.2），用临时 key/配额/别名实测：

| 项 | 结果 | 证据 |
|---|---|---|
| requests 配额（reject） | ✅ | 前两次 200 且 used=2/2；第三次 429 `quota exceeded (requests): 2 of 2 used`；审计记 `quota_exceeded`；手动 reset 后恢复 |
| tokens 配额（reject） | ✅ | 首次 200 并按估算计入 used（"hi"+max_tokens=10 → 23 tokens）；二次 429 `quota exceeded (tokens): 23 of 1 used` |
| 超限降级（degrade） | ✅ | glm5.2 配额 1 次，第 2 次不拒绝，自动切到备用别名 `diag-degrade`（审计 `model_alias=diag-degrade, ok`） |
| 速率限制（固定窗口） | ✅ | 2 次/60s，窗口内第 3 次 429 `rate limit rule#2: 2 reqs/60s` |

**两条运维特性（非缺陷，脚本化对接时需注意）**：
- 管理端创建/修改配额、限流规则后，快照热重载有 **~1-2s 延迟**——期间新规则不拦截、
  也不计数（第一轮实测的"失效"假象即由此而来：请求在规则进快照前发出）。
- 审计落库为异步批量：刚完成的调用出现在 call_logs 有 ~1s 延迟，且按 id 排序 ≠ 按完成
  时间排序。

回归脚本 `backend/test_quota.py`（未提交）：拉起 8081 自测全链路，含自动清理，可复跑。
